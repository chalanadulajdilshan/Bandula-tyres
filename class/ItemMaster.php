...
class ItemMaster
{
    ...
    public function getItemsByActiveBrands($category_id = 0, $brand_id = 0, $group_id = 0, $item_code = '')
    {
        $db = Database::getInstance();
        $conditions = ["`im`.`is_active` = 1"];

        // Only show items whose brand is active
        ...
        if (!empty($item_code)) {
            $escaped = $db->escapeString($item_code);
            $conditions[] = "(`im`.`code` LIKE '%{$escaped}%' OR `im`.`name` LIKE '%{$escaped}%')";
        }

        $where = "WHERE " . implode(" AND ", $conditions);

        $query = "SELECT im.*, b.name AS brand_name, c.name AS category_name, g.name AS group_name
                  FROM item_master im
                  INNER JOIN brands b ON im.brand = b.id
                  LEFT JOIN category_master c ON im.category = c.id
                  LEFT JOIN group_master g ON im.`group` = g.id
                  {$where}
                  ORDER BY b.name ASC, im.name ASC";
