jQuery(document).ready(function () {
  ...
  function processInvoiceCreation() {
    ...
    $("#invoiceItemsBody tr").each(function () {
      ...
      const vat_amount =
        parseFloat($(this).find('input[name="item_vat_amount[]"]').val()) || 0;
      const service_qty =
  ...
  function addItem() {
    ...
    if (hasService && hasServiceItem) {
      ...
      })}<input type="hidden" name="item_vat_amount[]" value="${serviceVatAmount.toFixed(2)}"></td>
      ...
      const serviceItemRow = `
        ...
        minimumFractionDigits: 2,
        maximumFractionDigits: 2,
      })}<input type="hidden" name="item_vat_amount[]" value="${serviceItemVatAmount.toFixed(2)}"></td>
      ...
      arnAllocations.forEach(function (allocation) {
        ...
        const row = `
            ...
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          })}<input type="hidden" name="item_vat_amount[]" value="${itemVatAmount.toFixed(2)}"></td>
                            ...
                            <tr>
                                ...
                                <td class="item-vat-amount vat-column" style="display: ${$("#is_vat_invoice").is(":checked") ? "" : "none"}">
                                  ${(() => {
                                    const isVatApplied = $("#is_vat_invoice").is(":checked");
                                    const vatPercentage = parseFloat($("#vat_percentage").val()) || 0;
                                    if (!isVatApplied || vatPercentage <= 0) return "0.00";
                                    const net = (price - discount) * qty;
                                    const vat = net * (vatPercentage / (100 + vatPercentage));
                                    return vat.toFixed(2);
                                  })()}
                                  <input type="hidden" name="item_vat_amount[]" value="${(() => {
                                    const isVatApplied = $("#is_vat_invoice").is(":checked");
                                    const vatPercentage = parseFloat($("#vat_percentage").val()) || 0;
                                    if (!isVatApplied || vatPercentage <= 0) return 0;
                                    const net = (price - discount) * qty;
                                    const vat = net * (vatPercentage / (100 + vatPercentage));
                                    return vat.toFixed(2);
                                  })()}">
